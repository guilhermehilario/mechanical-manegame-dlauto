...
| 2026-09-18 | G3 | gate de audit só planejado | `pnpm audit --prod --audit-level=high` (falha em high/critical; resíduo moderado aceito) — local + job `audit` do CI | audit exit 0 | gate ativo |
| 2026-09-18 | G4 | "CI local" (husky/turbo) | `.github/workflows/ci.yml` (audit, quality, e2e, smoke) — validações locais verdes | lint/typecheck/test/build + e2e 10/10 + smoke ALL CHECKS PASSED | pronta; ativação via push/actions |
